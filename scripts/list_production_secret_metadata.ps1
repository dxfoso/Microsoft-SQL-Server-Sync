...
set -euo pipefail
namespace="$1"
kubectl get secrets -n "$namespace" -o go-template='{{range .items}}{{.metadata.name}}{{"\t"}}{{.type}}{{"\t"}}{{index .metadata.annotations "kubernetes.io/service-account.name"}}{{"\t"}}{{range $key, $_ := .data}}{{$key}}{{"\t"}}{{end}}{{"\n"}}{{end}}'
'@
