...
| Replace/revoke external container-registry credential | **Blocked - registry owner must issue a new credential; Cloud APIs are forbidden by repository policy** | 0% |
...
- **Remaining security blocker (INC-409):** all internally controlled exposed credentials are rotated or revoked and verified. Only `regcred` remains because a replacement container-registry credential must be issued by the registry owner; repository policy forbids Cloud API/token use. Existing workloads are healthy, but a future image pull must not be attempted after revoking the old credential until its replacement is installed.
