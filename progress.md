...
- Live deletion diagnosis found c1 tombstones were uploaded but quarantined on c2 because its SQL Server rejects `THROW`; client `1.0.155+159` replaces it with compatible `RAISERROR` and prevents quarantined rows from advancing the checkpoint. Publication and live deletion retest are pending.
