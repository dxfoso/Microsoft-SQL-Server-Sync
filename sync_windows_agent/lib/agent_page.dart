...
''';
  ...
  Future<void> _openCreateClientInstanceDialog() async {
          ...
          (context) => StatefulBuilder(
            builder:
                (context, setDialogState) => AlertDialog(
                  title: const Text('Add separate SQL client'),
                  content: SizedBox(
                    width: 440,
                    child: Column(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        const Text(
                          'This creates an isolated client for another SQL Server/database on this PC. It uses the same login but has independent sync state and logs.',
                        ),
                        ...
                        TextField(
                          ...
                          decoration: _compactInputDecoration(
                            'Display name',
                          ).copyWith(hintText: 'Alshallan2 SQL8'),
                        ),
                        const SizedBox(height: 12),
                        TextField(
                          ...
                          decoration: _compactInputDecoration(
                            'SQL Server',
                          ).copyWith(hintText: r'DESKTOP-NAME\SQLEXPRESS'),
                        ),
                        const SizedBox(height: 12),
                        TextField(
                          ...
                          decoration: _compactInputDecoration(
                            'Database',
                          ).copyWith(hintText: 'Database name'),
                        ),
                        if (errorText != null) ...[
                    ...
                    FilledButton.icon(
                      onPressed:
                          creating
                              ...
                              : () async {
                                final id =
                                    instanceIdController.text
                                        .trim()
                                        .toLowerCase();
                                final displayName =
                                    displayNameController.text.trim();
                                final server = serverController.text.trim();
                                final database = databaseController.text.trim();
                                if (!RegExp(
                                  ...
                                  setDialogState(() {
                                    errorText =
                                        'Enter a valid ID, display name, SQL Server, and database.';
                                  });
                                  return;
                                }
                                setDialogState(() {
                                ...
                                try {
                                  await WindowsAgentWindowSettings.createClientInstance(
                                    instanceId: id,
                                  ...
                                  ScaffoldMessenger.of(
                                    ...
                                    SnackBar(
                                      content: Text(
                                        '$displayName was created and is starting in the background.',
                                      ),
                                    ),
                                  );
                                } catch (error) {
                      ...
                      label: Text(creating ? 'Creating...' : 'Create client'),
                    ),
                  ],
                ),
          ),
                    ...
                    children: [
                      ...
                      Align(
                        ...
                        child: OutlinedButton.icon(
                          onPressed: () {
                            Navigator.of(context).pop();
                            unawaited(_openCreateClientInstanceDialog());
                          },
                          icon: const Icon(Icons.add_to_queue, size: 16),
                          label: const Text('Add separate SQL client'),
                        ),
                      ),
                      const SizedBox(height: 4),
                      Align(
    ...
    final actions = Wrap(
      ...
      children: [
        ...
        FilledButton.tonalIcon(
          onPressed:
              _databaseAccessGrantBusy || grantableIssues.isEmpty
                  ? null
