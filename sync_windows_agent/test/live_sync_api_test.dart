...
class _ScriptedClient extends http.BaseClient {
  ...
  final ({
    int statusCode,
    Object? body,
  }) Function(String name, Map<String, dynamic> args, int callIndex)
  responseForRequest;
  ...
  Future<http.StreamedResponse> send(http.BaseRequest request) async {
    ...
    final body =
        response.body is String ? response.body as String : jsonEncode(response.body);
    return http.StreamedResponse(
      Stream<List<int>>.value(utf8.encode(body)),
      response.statusCode,
      headers: const {'content-type': 'application/json'},
...
void main() {
  ...
  test('downloadSnapshot retries transient 503 manifest failures and succeeds', () async {
    ...
    expect(
      client.requests.map((item) => item['name']).toList(),
      ['jobs_download_snapshot_manifest', 'jobs_download_snapshot_manifest', 'jobs_download_snapshot_chunk'],
    );
  });

  test('uploadSnapshot retries transient 503 start failures and finalizes resumed uploads', () async {
    final client = _ScriptedClient(
      responseForRequest: (name, args, callIndex) {
        switch (name) {
    ...
    expect(
      client.requests.map((item) => item['name']).toList(),
      ['jobs_upload_chunk_start', 'jobs_upload_chunk_start', 'jobs_upload_chunk_complete'],
    );
  ...
  test('downloadSnapshot retries transient 503 during chunk download and preserves all rows', () async {
    ...
    expect(
      client.requests.map((item) => '${item['name']}:${(item['args']['chunkIndex'] ?? '')}').toList(),
      [
  ...
  test('uploadSnapshot retries transient 503 during chunk upload and resumes remaining chunks', () async {
    ...
    expect(
      client.requests.where((item) => item['name'] == 'jobs_upload_chunk').length,
    ...
    expect(progressEvents, isNotEmpty);
    expect(progressEvents.last.bytesTransferred, progressEvents.last.totalBytes);
  });

  test('uploadSnapshot skips chunks already acknowledged by the server resume state', () async {
    final snapshotJson = _buildLargeSnapshotJson(
      clientName: 'c1',
      table: 'db::gr000',
      rowCount: 20000,
    ...
    final uploadedChunkIndexes = client.requests
        .where((item) => item['name'] == 'jobs_upload_chunk')
        .map((item) => item['args']['chunkIndex'])
        .toList();
  ...
  test('downloadSnapshot rejects incomplete manifests before reading chunks', () async {
    final client = _ScriptedClient(
      responseForRequest: (name, args, callIndex) {
  ...
  test('downloadSnapshot rejects payloads whose byte count does not match the manifest', () async {
    final snapshotJson = _buildLargeSnapshotJson(
      clientName: 'c2',
      table: 'db::gr000',
      rowCount: 10,
    ...
    await expectLater(
      ...
      throwsA(
        isA<AgentControlPlaneException>().having(
          (error) => error.message,
          'message',
          contains('Downloaded snapshot byte count does not match the manifest.'),
        ),
      ),
    );
  });
  ...
  test('downloadSnapshot rejects decompressed payloads that are not JSON objects', () async {
    final compressed = gzip.encode(utf8.encode(jsonEncode(['not', 'a', 'map'])));
    final client = _ScriptedClient(
      responseForRequest: (name, args, callIndex) {
        switch (name) {
          case 'jobs_download_snapshot_manifest':
  ...
  test('downloadSnapshot rejects decompressed payloads whose rows are not maps', () async {
    final snapshotJson = jsonEncode({
      'id': 'snap-bad-rows',
      'columns': const ['id'],
          ...
          case 'jobs_download_snapshot_chunk':
            return (
              ...
              body: {
                'status': 'success',
                'value': {
                  'chunkData': base64Encode(compressed),
                },
              },
            );
          default:
  ...
  test('downloadSnapshot rejects malformed manifest snapshot metadata field types', () async {
    final snapshotJson = jsonEncode({
      'id': '',
      'clientName': '',
      'table': 'db::mt000',
  ...
  test('uploadSnapshot rejects malformed upload completion JSON', () async {
    final client = MockClient((request) async {
      final payload = jsonDecode(request.body as String) as Map<String, dynamic>;
  ...
  test('uploadSnapshot rejects malformed snapshot payloads in completion', () async {
    final client = _ScriptedClient(
      responseForRequest: (name, args, callIndex) {
        switch (name) {
          case 'jobs_upload_chunk_start':
  ...
  test('uploadSnapshot rejects malformed job field types in completion', () async {
    final client = _ScriptedClient(
      responseForRequest: (name, args, callIndex) {
        switch (name) {
          case 'jobs_upload_chunk_start':
  ...
  test('fetchClientUpdateInfo rejects malformed manifest field types', () async {
    final manifestApi = AgentControlPlaneClient(
      client: MockClient((request) async {
        return http.Response(
          jsonEncode({
  ...
  test('acknowledgeClientUpdate rejects malformed acknowledgement payloads', () async {
    final client = _DelayedClient(
      delay: Duration.zero,
      responseForName: (name) {
  ...
  test('acknowledgeClientUpdate rejects malformed acknowledgement field types', () async {
    final client = _DelayedClient(
      delay: Duration.zero,
      responseForName: (name) {
        expect(name, 'agent_client_update_ack');
  ...
  test('acknowledgeWindowAction rejects malformed acknowledgement payloads', () async {
    final client = _DelayedClient(
      delay: Duration.zero,
      responseForName: (name) {
  ...
  test('acknowledgeWindowAction rejects malformed acknowledgement field types', () async {
    final client = _DelayedClient(
      delay: Duration.zero,
      responseForName: (name) {
        expect(name, 'agent_window_action_ack');
  ...
  test('fetchClientUpdateInfo returns null for 404 and parses a valid manifest', () async {
  ...
  test('checkHealth returns false for non-200 responses and request failures', () async {
  ...
  test('logout calls auth_logout once and clears the stored auth token', () async {
    final client = _ScriptedClient(
      responseForRequest: (name, args, callIndex) {
  ...
  test('failJob posts the expected payload including optional progress', () async {
    final client = _ScriptedClient(
      responseForRequest: (name, args, callIndex) {
  ...
  test('job lifecycle methods parse valid job payloads', () async {
    Map<String, Object?> jobPayload(String status, int progress, String message) => {
      'job': {
        'id': 'job-1',
        'clientName': 'c1',
        'sourceClientName': 'c1',
  ...
  test('base URL normalization falls back to the live control plane for localhost-style inputs', () {
    final blank = AgentControlPlaneClient(baseUrl: '');
    final localhost = AgentControlPlaneClient(baseUrl: 'http://127.0.0.1:6006/call');
    final hostPortOnly = AgentControlPlaneClient(baseUrl: 'https://example.com:6006/call');
    final trailingSlash = AgentControlPlaneClient(baseUrl: 'https://sync.velvet-leaf.com/call/');
    final custom = AgentControlPlaneClient(baseUrl: 'https://example.com/custom-call');
  ...
  test('authenticated function calls add token to args and skip auth_login injection', () async {
    final client = _ScriptedClient(
      responseForRequest: (name, args, callIndex) {
        if (name == 'auth_login') {
    ...
    expect(client.requests.map((item) => item['name']).toList(), ['auth_login', 'jobs_fail']);
  });

  test('fetchClientUpdateInfo surfaces structured and generic HTTP errors clearly', () async {
  ...
  test('downloadSnapshot does not retry non-retryable manifest failures', () async {
    var requestCount = 0;
    final client = _ScriptedClient(
      responseForRequest: (name, args, callIndex) {
        requestCount += 1;
  ...
  test('downloadSnapshot surfaces retry exhaustion for manifest failures', () async {
    var requestCount = 0;
    final client = _ScriptedClient(
      responseForRequest: (name, args, callIndex) {
        requestCount += 1;
  ...
  test('downloadSnapshot surfaces retry exhaustion for chunk failures', () async {
        ...
        switch (name) {
          ...
          case 'jobs_download_snapshot_chunk':
            ...
            return (
              statusCode: 503,
              body: {
                'error': 'chunk temporarily unavailable',
              },
            );
          default:
            fail('Unexpected request $name');
  ...
  test('uploadSnapshot stops immediately on non-retryable chunk upload failures', () async {
    var chunkRequestCount = 0;
    final client = _ScriptedClient(
      responseForRequest: (name, args, callIndex) {
        switch (name) {
          ...
          case 'jobs_upload_chunk':
            ...
            return (
              statusCode: 401,
              body: {
                'error': 'chunk upload unauthorized',
              },
            );
          default:
            fail('Unexpected request $name');
  ...
  test('uploadSnapshot surfaces retry exhaustion for chunk upload failures', () async {
    var chunkRequestCount = 0;
    final client = _ScriptedClient(
      responseForRequest: (name, args, callIndex) {
        switch (name) {
          ...
          case 'jobs_upload_chunk':
            ...
            return (
              statusCode: 503,
              body: {
                'error': 'chunk upload unavailable',
              },
            );
          default:
            fail('Unexpected request $name');
