...
class ControlPlanePerfContractsTests(unittest.TestCase):
    ...
    def test_live_state_aggregates_completed_row_totals_in_one_scoped_read(self):
        ...
        self.assertIn("status: 'completed'", totals_body)
        self.assertIn("where: { direction: 'upload' }", totals_body)
        self.assertIn("where: { direction: 'download' }", totals_body)
        self.assertIn("groupBy: ['clientName']", totals_body)
    ...
    def test_sync_all_loads_large_agent_table_state_one_owner_at_a_time(self):
        ...
        )[1].split("function find_agent_row_by_name", 1)[0]
        ...
        self.assertNotIn("let skippedOfflineClients = []", sync_all)
        self.assertNotIn("let sourceResolutionTables = []", sync_all)
