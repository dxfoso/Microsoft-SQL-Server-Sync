...
class SyncContractsTests(unittest.TestCase):
    ...
    def test_requested_client_logs_are_detailed_complete_and_redacted(self):
        ...
        self.assertIn("'sync.download.winner_filter_completed'", client_api)
        self.assertIn("winnerPolicyCandidateRowCount > 0", client_api)
        self.assertIn("winnerPolicyAcceptedRowCount == 0", client_api)
        self.assertIn("shouldRetainAgentDiagnostic", startup_log)
        self.assertIn("_routineHighVolumeDebugEvents", startup_log)
    ...
    def test_web_workspace_has_dashboard_and_client_log_navigation(self):
        ...
        self.assertIn("String get _sectionTitle => _isAdmin ? 'Users' : 'Operations'", app)
        self.assertIn("Icons.dashboard_outlined", app)
        self.assertIn("_navItem(", app)
        ...
        self.assertIn("Table actions", clients_page)
        ...
        self.assertIn("ValueKey('sync-last-activity-${agent.clientName}')", clients_page)
        self.assertIn("ValueKey('sync-last-result-${agent.clientName}')", clients_page)
        self.assertIn("ValueKey('sync-totals-${agent.clientName}')", clients_page)
        self.assertNotIn("DataColumn(label: Text('Row changes'))", clients_page)
        self.assertIn("_buildLastResultCell(agent)", clients_page)
        self.assertIn("_buildSyncTotalsCell(agent)", clients_page)
        self.assertIn("_buildLastActivityCell(agent)", clients_page)
        self.assertIn("_buildSyncDurationCell(agent)", clients_page)
        self.assertIn("ValueKey('sync-duration-${agent.clientName}')", clients_page)
        self.assertIn("formatSyncDuration(operation?.duration())", clients_page)
    ...
    def test_web_download_resolves_the_latest_immutable_windows_client(self):
        ...
        self.assertIn("version.isEmpty ? 'Windows client' : 'Windows client · v$version'", app)
    ...
    def test_integrity_audit_is_durable_visible_and_bounded(self):
        ...
        self.assertIn("_buildLastResultCell(agent)", clients)
    ...
    def test_table_readiness_actions_are_fixed_visible_and_safely_labeled(self):
        ...
        self.assertIn("const actionColumnWidth = 304.0", grid)
        ...
        self.assertIn("ValueKey('table-actions-${table.table}')", actions)
        self.assertIn("tooltip: 'Table actions'", actions)
        self.assertIn("Text('Table actions')", actions)
        self.assertIn("'compare_rows'", actions)
        self.assertIn("Text('Compare client rows')", actions)
        self.assertIn("title: Text('Future changes only')", actions)
        self.assertIn("'accept_baseline'", actions)
        self.assertIn("title: Text(\n                  'Ignore table'", actions)
        self.assertIn("'exclude_table'", actions)
        self.assertIn("title: Text('Retry after local correction')", actions)
        self.assertNotIn("FilledButton.tonalIcon(", actions)
        self.assertNotIn("OutlinedButton.icon(", actions)
    ...
    def test_multi_writer_download_resolves_each_durable_marker_exactly(self):
        ...
        self.assertIn("SHA-256(owner + table + identity)", exact_id_helper)
        self.assertNotIn("winner.ownerUserId", exact_id_helper)
        self.assertNotIn("winner.table", exact_id_helper)
    ...
    def test_multi_writer_upload_retains_physical_winner_id_across_marker_scope(self):
        control_plane = read_text("business/control_plane.tru")
        upload = control_plane.split("function jobs_multi_writer_upload(", 1)[1].split(
            "function jobs_multi_writer_download(", 1
        )[0]
        candidate_loop = upload.split(
            "for (const incomingRow of effectiveIncomingRows)", 2
        )[2].split("if (pendingWinners.length > 0)", 1)[0]

        declaration = "let durableWinnerId = '';"
        policy_branch = "if (winnerPolicyApplied) {"
        assignment = "durableWinnerId = winnerId;"
        marker_append = "acceptedOperationIds = acceptedOperationIds.concat([{"
        self.assertIn(declaration, candidate_loop)
        self.assertIn(assignment, candidate_loop)
        self.assertLess(candidate_loop.index(declaration), candidate_loop.index(policy_branch))
        self.assertLess(candidate_loop.index(assignment), candidate_loop.index(marker_append))
        self.assertNotIn("winnerPolicyApplied ? winnerId", candidate_loop)
    ...
    def test_remote_database_clone_is_chunked_target_bound_and_fail_closed(self):
        agent_page = read_text("sync_windows_agent/lib/agent_page.dart")
        ...
        self.assertIn("if (!is_admin_user(current))", backend)
        self.assertIn("target.syncEnabled != false", backend)
        self.assertIn("source.isOnline != true || source.sqlConnected != true", backend)
        self.assertIn("sourceActiveJobs", backend)
        self.assertIn("targetActiveJobs", backend)
        self.assertIn("target client still has active synchronization jobs", backend)
        self.assertIn("source has no completed verified full backup export", backend)
        self.assertIn("normalizedSource == normalizedTarget", backend)
        self.assertIn("dataExportMode: 'full_restore'", backend)
        self.assertIn("dataExportSourceRequestId: sourceRequestId", backend)
        self.assertIn("A restore command's size/hash/chunk tuple is its immutable integrity", backend)
        self.assertIn("nextBytes = int.from(matchingAgent.dataExportBytes ?? 0)", backend)
        self.assertIn("nextSha256 = string.from(matchingAgent.dataExportSha256 ?? '')", backend)
        self.assertIn("nextChunkCount = int.from(matchingAgent.dataExportChunkCount ?? 0)", backend)
        self.assertIn("sourceRequestId", api)
        self.assertIn("sourceClientName", api)
        self.assertIn("_downloadPrivateExportArtifact", agent_page)
        self.assertIn("for (var index = 0; index < request.chunkCount", agent_page)
        self.assertIn("Creating verified rollback backup", agent_page)
        self.assertIn("buildReplaceDatabaseFromBackupSql", agent_page)
        self.assertIn("_recoverDatabaseFromRollback", agent_page)
        self.assertIn("Automatic rollback also failed", agent_page)
        self.assertIn("restored automatically and passed DBCC CHECKDB", agent_page)
        self.assertIn("DBCC CHECKDB", agent_page)
        self.assertIn("_resetSelectedDatabaseSyncBaselinesAfterReplacement", agent_page)
        self.assertIn("WITH REPLACE", policy)
        self.assertIn("SET SINGLE_USER WITH ROLLBACK IMMEDIATE", policy)
        self.assertIn("SET MULTI_USER WITH ROLLBACK IMMEDIATE", policy)
        self.assertIn("buildDatabaseStorageDirectoriesSql(sourceDatabase)", agent_page)
        ...
        self.assertIn("Assert-ClonePreconditions", orchestrator)
        self.assertIn("Target synchronization must be disabled", orchestrator)
        self.assertIn("agent_data_export_request", orchestrator)
        self.assertIn("agent_database_clone_request", orchestrator)
        self.assertIn("[switch] $ReuseCompletedSourceBackup", orchestrator)
        self.assertIn("Source has no completed verified full backup export to reuse.", orchestrator)
        self.assertIn("$sourceStatus -ne 'completed'", orchestrator)
        self.assertIn("$sourceMode -ne 'full_backup'", orchestrator)
        self.assertIn("$sourceSha256 -notmatch '^[0-9a-fA-F]{64}$'", orchestrator)
        self.assertIn("Wait-ForExport", orchestrator)
        self.assertNotIn("Write-Host $adminPassword", orchestrator)
        self.assertNotIn("Write-Host $privateToken", orchestrator)

    def test_isolated_alameen_lab_provisioning_is_non_destructive_and_audited(self):
        agent_page = read_text("sync_windows_agent/lib/agent_page.dart")
        policy = read_text("sync_windows_agent/lib/database_backup_restore.dart")
        api = read_text("sync_windows_agent/lib/live_sync_api.dart")
        backend = read_text("business/control_plane.tru")
        provision = read_text("scripts/provision_alameen_sync_lab.ps1")
        launcher = read_text("scripts/start_alameen_sync_lab_hidden.ps1")

        self.assertIn("function agent_database_lab_restore_request", backend)
        self.assertIn("dataExportMode: 'lab_restore'", backend)
        self.assertIn("dataExportTargetDatabase: normalizedTargetDatabase", backend)
        self.assertIn("source and target synchronization must be disabled", backend)
        self.assertIn("source has no completed verified full backup export", backend)
        self.assertIn("targetDatabase", api)
        self.assertIn("request.mode == 'full_restore' || request.mode == 'lab_restore'", agent_page)
        self.assertIn("buildRestoreAsNewDatabaseSql", agent_page)
        self.assertIn("buildInstallAlameenLabAuditSql", agent_page)
        self.assertIn("Isolated lab database", agent_page)
        self.assertIn("CREATE TABLE dbo.SqlSyncLabAudit", policy)
        self.assertIn("ENABLE CHANGE_TRACKING", policy)
        self.assertNotIn("DROP DATABASE", policy.split("String buildInstallAlameenLabAuditSql", 1)[1])
    ...
    def test_local_database_backup_restore_is_non_blocking_visible_and_non_destructive(self):
        agent_page = read_text("sync_windows_agent/lib/agent_page.dart")
        policy = read_text("sync_windows_agent/lib/database_backup_restore.dart")
        local_restore_policy = policy.split(
            "String buildRestoreAsNewDatabaseSql(", 1
        )[1].split("String buildReplaceDatabaseFromBackupSql(", 1)[0]
        runner = read_text("tests/run_sync_verification.ps1")
        ...
        self.assertIn("IF DB_ID(N'$databaseLiteral') IS NOT NULL", local_restore_policy)
        self.assertIn("RESTORE DATABASE [$escapedDatabase]", local_restore_policy)
        self.assertIn("CHECKSUM, RECOVERY, STATS = 5", local_restore_policy)
        self.assertNotIn("WITH REPLACE", local_restore_policy)
        ...
        self.assertIn(
            "full-database-copy-only-backup-verified-target-replacement-with-rollback",
            docker_scenarios,
        )
