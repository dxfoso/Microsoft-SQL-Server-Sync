...
class IncidentRegressionCatalogTests(unittest.TestCase):
    def test_every_catalog_incident_has_existing_automated_coverage(self):
        ...
        expected_ids = {f"INC-{number:03d}" for number in range(1, 497)}
    ...
    def test_live_client_update_launcher_is_hidden_secret_safe_and_quotes_names(self):
        launcher = read_text("scripts/start_live_client_update_hidden.ps1")
        publisher = read_text("scripts/publish_windows_client_update.ps1")

        self.assertIn("kubectl get secret $AdminSecretName -n $Namespace -o json", launcher)
        self.assertIn("verify_live_client_update.py", launcher)
        self.assertIn("-WindowStyle Hidden", launcher)
        self.assertIn("RedirectStandardOutput", launcher)
        self.assertIn("RedirectStandardError", launcher)
        ...
        self.assertNotIn("Write-Host $adminPassword", launcher)
        self.assertIn("$publicReady = $false", launcher)
        self.assertIn("$manifest.filesManifestUrl", launcher)
        self.assertIn("no update was requested", launcher)
    ...
    def test_cloud_bootstrap_registry_rotation_is_secret_safe_and_pull_verified(self):
        ...
        self.assertIn("$oldAuthProperty.Value.PSObject.Properties['username']", rotation)
        self.assertIn("$oldAuthProperty.Value.PSObject.Properties['auth']", rotation)
    ...
    def test_sync_credentials_are_hashed_and_source_bootstrap_is_removed(self):
        ...
        self.assertEqual(rotation.count("app = 'web'"), 2)
        self.assertNotIn("app = 'credential-rotation'", rotation)
    ...
    def test_postgres_credential_rotation_is_coordinated_and_rollback_capable(self):
        ...
        self.assertIn("$drainDeadline = [DateTime]::UtcNow.AddSeconds($DrainTimeoutSeconds)", rotation)
        self.assertIn("CronJobs did not drain before timeout; no credential was changed.", rotation)
    ...
    def test_retired_windows_runner_python_alias_bug_cannot_reenter_cloud_tests(self):
        ...
        worker_image = read_text("backend/.cloud-ci/images/tru-ci/Dockerfile")
        ci_task = read_text("backend/.cloud-ci/tasks/ci.sh")
        common = read_text("backend/.cloud-ci/tasks/lib/common.sh")

        self.assertFalse(legacy_test.exists())
        self.assertIn("python3 python3-pip", worker_image)
        self.assertIn("require_command python3", ci_task)
        self.assertIn("required command not found", common)
    ...
    def test_updater_rollback_observer_outlives_supervisor_retry_bound(self):
        ...
        self.assertIn("$deadline = [DateTime]::UtcNow.AddSeconds(150)", rollback_test)
        self.assertIn("complete two-launch product bound", rollback_test)
